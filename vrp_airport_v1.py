...
def plot_a_simple_map(ready_route, filename):
    ...
    y=[22.623]
    for j in range(0, len(ready_route)):
        x.append(ready_route[j].position[0] * multiplier)
        y.append(ready_route[j].position[1])
        if ready_route[j].service_type == True:
            label='r*'
        else:
            label='g*'
...
def calculate_cutomer_out(customer_out_calculating, whole_route):
    ...
    for i in range( 0, len( customer_out_calculating ) ):  # 表示现有需要计算的订单
        ...
        if (whole_route == None):
            ...
            route_tmp=someclass.Route()
            route_tmp.route_id=customer_out_calculated_route_id
            route_list_tmp=[]
            route_list_tmp.append( customer_out_calculating[0] )
            # expected_return_time=float("inf")
            time_list=[]
            expected_drop_time0=distance_dictionary['airport'+'_'+customer_out_calculating[0]]
            time_list.append(expected_drop_time0)
            route_tmp.expected_drop_time_list=time_list
            route_tmp.route_list=route_list_tmp
            route_tmp0=[]
                ...
                for j in range( 0, len(a.route_list) + 1 ):
                    if j == 0:
                        distance_increase = distance_dictionary['airport' + '_' + customer_out_calculating[i].id] + \
                                            distance_dictionary[
                                                customer_out_calculating[i].id + '_' + a.route_list[0].id] - \
            ...
            temp_route = insert_route.route_list(insert_position, customer_out_calculating[i])
            # customer_out_calculated[a].insert( j, customer_out_calculating[i] )
            can_insert, time_list = check2distance.check2distance(temp_route)
            if can_insert == 0:
                ...
                whole_route[0][insert_route_index].route_drop_time_list=time_list
    whole_route[1] = whole_route[0]
    whole_route[0] = None

    return whole_route, customer_out_cannot_service

def calculate_cutomer_in(customer_in_calculating, whole_route):
    distance_test = float( "inf" )
    for i in range( 0, len(customer_in_calculating)):  # 表示现有需要计算的订单
        for a in whole_route[1]:
            if customer_in_calculating[i].on_time < a.route_drop_time_list[-1]:
                if customer_in_calculating[i].on_time < a.route_drop_time_list[0]:
                    ...
                    for j in range( 0, len( a.route_drop_time_list ) - 1 ):
                        if (customer_in_calculating[i].on_time > a.route_drop_time_list[j]) and \
                                (customer_in_calculating[i].on_time < a.route_drop_time_list[j+1]):
                            if j==len(a.route_drop_time_list)-2:
                                distance_increase = distance_dictionary[a.route_list[-1]+'_'+customer_in_calculating[j]]+ \
                                ...
                                distance_increase = distance_dictionary[
                                                    ...
                                                    distance_dictionary[
                                                        a.route_drop_time_list[j].id + '_' +a.route_drop_time_list[j + 1].id]
        ...
        if can_insert == 0:
            ...
            whole_route[1][insert_route_index].route_drop_time_list = time_list
...
if __name__=="__main__":
    ...
    customer_out=read_file.read_file(filename_out,True)
    customer_in=read_file.read_file(filename_in,False)
...
#    customer_out_calculated=dict()#有顺序的各种不同线路
    ...
    for timestamp_1 in range(1438531200, 1438617600, 900):
        ...
        f.write( "此时段的出机场订单：" + str(customer_out_calculating)+'\n')
        ...
        for a in whole_route[1]:
            if a.route_drop_time_list[-1]>latest_return_time:
                latest_return_time=a.route_drop_time_list[-1]
        ...
        f.write( "此时段的入机场订单可能有：" + str(customer_in_calculating)+'\n')
