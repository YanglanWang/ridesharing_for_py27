...
def VNS_calculate(initialSolution, customer, distance_matrix, distance_airport, time_matrix, time_airport, number_out):
    ...
    while checked_iteration_times < maxIteration:
        ...
        if len(finalSolution) > 1:
            ...
            end_inter_route = time.clock()
            f.write( "线路间循环所需时间：" + str(end_inter_route-start_inter_route)+'\n' + '\n')
        ...
        #检查线路内是否发生变化
        #for i in range(0, len(finalSolution)):
            #if initialSolution[0][i]!=finalSolution[i]:
                #intra_change_times+=1
        #if intra_change_times>=1:
            #f.write("线路内循环有改变")
        #else:
            #f.write("线路内循环无改变")
            
        end_intra_route=time.clock()
        f.write( "线路内循环所需时间：" + str(end_intra_route-start_intra_route)+'\n' + '\n')
    
    
    f.write( "实际循环次数：" + str(iteration_times+1) + '\n')
...
if __name__=="__main__":
    ...
    f=open('both mode_try0602(3)maxiteration_4_0.7.txt', 'w')
    f.write( "2015-08-03" + '\n')
    
    g=open('both mode_try0602_brief(3)maxiteration_4_0.7.txt', 'w')
    g.write( "2015-08-03" + '\n')
    ...
    for timestamp_1 in range(1438531200-1438500000, 1438617600-1438500000-7200, 300):
        ...
        g.write( timestr_1 + "--" + timestr_2 + ';')
        print "当前时间：" + timestr_1 + "--" + timestr_2
        
        """
        ======= 检验上一轮循环中的route，到时间的route则发车输出 ========
        """
        start_check_routeout = time.clock()
        
        customer_served=0 #用来表示这一时段被服务的顾客总数
        n_tmp_1 = 0
        n_tmp_2 = 0
        ...
        if len(current_solution) != 0:
            ...
            for i in range(0, len(current_solution)):
                if start_time[i][0] - 600 < timestamp_2:
                    ...
                    #customer_served=customer_served+len(current_solution[i])-2
            ...
            if len(deleted_customer) == 0:
                ...
                customer_served=len(deleted_customer)
                #f.write( "此次循环完成的出机场订单： " + str(n_tmp_1)+"  此次循环完成的入机场订单： " + str(n_tmp_2)+" 总订单："+str(customer_served)+'\n')
        ...
        end_check_routeout = time.clock()
        f.write( "检查上一轮的route是否到时间发车所需时间：" + str(end_check_routeout-start_check_routeout) +'\n')      
        
        f.write( "此次循环完成的出机场订单：" + str(n_tmp_1)+" 此次循环完成的入机场订单：" + str(n_tmp_2)+" 总订单："+str(customer_served)+'\n')

        
        customer_accumulated = len(current_customer) #前几轮累计的顾客数
        f.write( "完成此次循环后累积的出机场订单：" + str(number_out)+" 入机场订单：" + str(number_in)+" 总订单" + str(customer_accumulated)+'\n')
        ...
        输入： customer_ID_out, on_GPS_out, total_off, customer_ID_in, on_GPS_in, total_on
        """
        start_check_neworder = time.clock()
        new_customer_ID_out = []
        new_time_window_out = []
        new_location_out = []
        """ 半小时后（假设的预约提前量）出发的订单"""
        ...
        end_check_neworder = time.clock()
        f.write( "收取新订单所需时间：" + str(end_check_neworder-start_check_neworder) +'\n')
        """
        ================== 更新矩阵---添加 =====================
        ...
        f.write( "新出机场订单数：" + str(new_number_out) +"  新入机场订单数量：" + str(new_number_in) +"  新出入机场的总订单数：" + str(new_number_in+new_number_out) +'\n')
        
        
        if (new_number_out + new_number_in) != 0:  # 不为0，说明有顾客来；若为0，没新顾客来，矩阵不变 
            
            # 若当前矩阵为空，则从头构建矩阵
            if len(current_customer) == 0:
                if len(current_solution) == 0:
                    f.write( "因为前几轮没有留下未分配订单"+'\n')
                else:
                    f.write( "因为前几轮有未分配订单，但发车后已无剩余"+'\n')
                
                # 更新distance_airport, time_airport
                for i in range(0, new_number_out):
                    dis_tmp, time_tmp = manhattan_calculate([113.814, 22.623], new_location_out[i])
                    dis_tmp_2, time_tmp_2 = manhattan_calculate(new_location_out[i], [113.814, 22.623])
                ...
                for i in range(0, new_number_in):
                    dis_tmp, time_tmp = manhattan_calculate([113.814, 22.623], new_location_in[i])
                    dis_tmp_2, time_tmp_2 = manhattan_calculate(new_location_in[i], [113.814, 22.623])
                ...
                f.write("超时出机场订单数为："+str(len(unvalid_orders))+'\n')
                ...
                #customer_new_valid_order=number_tmp
                f.write( "删掉超时订单后的新出机场订单数：" + str(new_number_out) +"新入机场订单数：" + str(new_number_in) +"新出入机场总订单数：" + str(number_tmp) +'\n')
                ...
                for i in range(0, number_tmp):
                    for j in range(0, number_tmp):
                        if i != j:
                            ...
                            dis_tmp, time_tmp = manhattan_calculate(position_1, position_2)
            ...
            # 当前矩阵不为空，则把新数据添加进去
                ...
                #customer_new_valid_order = new_number_in+new_number_out
                #f.write( "删掉超时订单后的新出入机场总订单数：" + str(customer_new_valid_order) + '\n')
                ...
                for i in range(0, number_out):
                    ...
                    for j in range(0, new_number_out):
                        dis_tmp, time_tmp = manhattan_calculate(current_customer[i][3], new_location_out[j])
                        distance_matrix[i].insert(number_out + j, dis_tmp)
                        time_matrix[i].insert(number_out + j, time_tmp)
                        num_of_API_used_tmp += 1 #记录次数
                    
                    for j in range(0, new_number_in):
                        dis_tmp, time_tmp = manhattan_calculate(current_customer[i][3], new_location_in[j])
                ...
                for i in range(0, new_number_out):
                    ...
                    for j in range(0, n_total):
                        if j == i + number_out:
                            ...
                            dis_tmp, time_tmp = manhattan_calculate(new_location_out[i], current_customer[j][3])
                ...
                for i in range(0, new_number_in):
                    ...
                    for j in range(0, n_total):
                        if j == i + number_out + new_number_out + number_in:
                            ...
                            dis_tmp, time_tmp = manhattan_calculate(new_location_in[i], current_customer[j][3])
                            d_tmp.append(dis_tmp)
                            t_tmp.append(time_tmp)
                            num_of_API_used_tmp += 1 #记录次数
            ...
            #customer_new_valid_order=0
            #f.write( "没有新订单"+'\n')
            if len(current_customer) == 0:
                if len(current_solution) == 0:
                    f.write( "前几轮也没有留下未分配订单"+'\n')
                else:
                    f.write( "前几轮有未分配订单，但发车后已无剩余"+'\n')
           # else:
        ...
        f.write( "形成或更新矩阵所需时间：" + str(end_time_matrix-start_time_matrix) +'\n')
        ...
        #customer_need_calculate=customer_accumulated+customer_new_valid_order-customer_served
        ...
        if len(current_customer) == 0:
            ...
            # current_solution = []
            # start_time = []
            f.write( "本轮计算的点数中出机场的订单数为：" + str(number_out)+" 入机场的订单数为：" +str(number_in)+"出入机场的总订单数为："+ str(len(current_customer))+'\n')
            g.write(str(len(current_customer))+';')
            ...
            end_initial=time.clock()
            f.write( "初始解所需时间：" + str(end_initial-start_initial) +'\n')
            g.write(str(end_initial-start_initial)+';')
            
            start_VNS=time.clock()
            ...
            end_VNS=time.clock()
            f.write( "最终解所需时间：" + str(end_VNS-start_VNS) +'\n')
            g.write(str(end_VNS-start_VNS)+'\n')
